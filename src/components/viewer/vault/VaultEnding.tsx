...
const VaultEnding = ({ letterId }: VaultEndingProps) => {
  ...
  const handleSaveKeepsake = async () => {
    try {
      ...
      setSaved(true);
      toast.success("Link copied — bookmark it to keep this letter forever 💕");
      setTimeout(() => setSaved(false), 2500);
    } catch {
      toast.error("Couldn't copy the link. You can bookmark this page instead.");
    }
  };
  ...
  const handleSharePrivately = async () => {
    const shareData = {
      ...
      text: "I wanted you to see this letter someone wrote.",
    ...
    if (navigator.share) {
      try {
        ...
        if ((e as Error)?.name !== "AbortError") {
          toast.error("Couldn't open the share sheet. Try copying the link instead.");
        }
      }
      return;
    }
    try {
      await navigator.clipboard.writeText(window.location.href);
      toast.success("Link copied — share it with whoever you'd like 💌");
    } catch {
